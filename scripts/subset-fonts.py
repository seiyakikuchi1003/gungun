...
# ★漢字はソース収集だけでは足りない。
#   商品名・自己紹介・コメントはユーザーが自由に入力するため、
#   ソースに出てこない漢字が必ず現れる。欠けるとその字だけ別フォントになり、
#   実際に「閲覧」の字が崩れて見えていた（2026-08-05 指摘）。
#   CJK統合漢字を丸ごと対象にする（フォントが持たない字は自動で無視される）。
rng(0x4E00, 0x9FFF)  # CJK統合漢字
rng(0x3400, 0x4DBF)  # CJK統合漢字拡張A（稀な字。持っていれば入る）
for c in '①②③④⑤⑥⑦⑧⑨⑩★☆♪→←↑↓♥※〒℃…‥、。「」『』【】〜ー':
    chars.add(c)
