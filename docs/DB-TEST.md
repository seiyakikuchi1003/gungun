...
実データ版のログイン画面には **「● 実データ（Supabase）に接続中」** というバッジが出ます。
モック版には何も出ません（先方デモの邪魔にならないように）。
見た目が同じ2つのURLなので、迷ったらここを見てください。
...
`.env` を自動で退避してからビルドし、終わったら戻します。
書き出したファイルに Supabase のキーが混ざっていないかを検査してから公開します。

> **手動で `npx expo export` しないこと。**
> Metro のビルドキャッシュには前回埋め込まれた `EXPO_PUBLIC_*` の値が残るため、
> `.env` を消しただけではモック版に接続情報が入り込みます（実際に一度混入しました）。
> 両スクリプトは `--clear` を付けてキャッシュを捨ててからビルドしています。
